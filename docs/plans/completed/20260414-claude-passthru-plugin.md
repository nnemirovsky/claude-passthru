...
- [x] create `hooks.json` with one `PreToolUse` entry, matcher `"*"`, pointing at `${CLAUDE_PLUGIN_ROOT}/hooks/handlers/pre-tool-use.sh`, timeout 10 (jq+perl per-rule overhead means 40-50 rules can approach 2s; 10s leaves slack for large rule sets without masking truly runaway hooks).
...
- Modify: `/Users/nemirovsky/Developer/claude-passthru/hooks/hooks.json` (add PostToolUse entry, matcher `"*"`, timeout 10s - same as PreToolUse to leave headroom for the breadcrumb sha snapshot, settings.json read, and audit JSONL append on slow filesystems)
...
- [x] run full test suite: `bats tests/*.bats`. Confirmed in this task: 257 tests, 0 failures (initial). After review phases the suite grew to 323 tests, 0 failures.
