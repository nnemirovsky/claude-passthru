---
description: Add a passthru allow or deny rule for a tool
argument-hint: "<scope> <tool> <pattern> [reason] [--deny] [--field <name>]"
---

# /passthru:add

Add a regex rule to the passthru allow or deny list for the given scope.

The plugin's `PreToolUse` hook evaluates passthru rules on every tool call and
auto-allows or auto-denies matching calls, bypassing the native permission
dialog. Use this command to add a rule from within a live session without
hand-editing `passthru.json`.
...
tokens in this order, with two flags allowed anywhere:
...
Where `<allow|deny>` is `deny` if `--deny` was present anywhere in the
arguments, otherwise `allow`. Single-quote the rule JSON so the shell does
not reinterpret it.
